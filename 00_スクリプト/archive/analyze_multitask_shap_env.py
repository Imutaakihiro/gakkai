...
# -*- coding: utf-8 -*-
"""
環境変数対応マルチタスク学習SHAP分析
感情スコア予測と授業評価スコア予測の要因分析（PyTorch 2.3.1対応版）
"""

import os
# PyTorchの安全性チェックを無効化（研究用途）
...
def stratified_sampling(df, n_samples=100):
...
def create_visualizations(sentiment_shap_dict, course_shap_dict, categories, output_dir):
    ...
    plt.xlabel('SHAP値')
    plt.title('感情スコア予測要因TOP20 (環境変数対応)')
    plt.tight_layout()
    plt.savefig(f"{output_dir}/sentiment_top20_factors_env.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.yticks(range(len(words)), words)
    plt.xlabel('SHAP値')
    plt.title('授業評価スコア予測要因TOP20 (環境変数対応)')
    plt.tight_layout()
    plt.savefig(f"{output_dir}/course_top20_factors_env.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('2タスクのSHAP値散布図 (環境変数対応)')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/factor_comparison_scatter_env.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('カテゴリ別要因数 (環境変数対応)')
    plt.ylabel('要因数')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/factor_categories_chart_env.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def save_results(sentiment_shap_dict, course_shap_dict, categories, output_dir):
    ...
    sentiment_df.to_csv(f"{output_dir}/word_importance_sentiment_env.csv", index=False, encoding='utf-8')
    ...
    course_df.to_csv(f"{output_dir}/word_importance_course_env.csv", index=False, encoding='utf-8')
    ...
    with open(f"{output_dir}/factor_categories_env.json", 'w', encoding='utf-8') as f:
    ...
    with open(f"{output_dir}/analysis_summary_env.json", 'w', encoding='utf-8') as f:
...
def create_summary_report(categories, output_dir):
    ...
    report = f"""# 環境変数対応マルチタスク学習SHAP分析結果サマリー

## 分析概要
- 分析日時: {datetime.now().strftime('%Y年%m月%d日 %H:%M:%S')}
- 分析対象: 授業レベルマルチタスク学習モデル（環境変数対応）
- サンプル数: 100件（層化サンプリング）
- 使用デバイス: {device}
...
"""
    ...
    with open(f"{output_dir}/multitask_shap_analysis_summary_env.md", 'w', encoding='utf-8') as f:
        f.write(report)
...
def main():
    """メイン処理（環境変数対応）"""
    print("🚀 環境変数対応マルチタスクSHAP分析を開始...")
    
    # 出力ディレクトリの作成
    output_dir = "../03_分析結果/マルチタスクSHAP分析_環境変数対応"
    ...
    # 層化サンプリング
    sampled_df = stratified_sampling(df, n_samples=100)
    
    # 2. モデルとトークナイザーの読み込み
    print("\n=== Phase 2: モデル読み込み（環境変数対応） ===")
    ...
    shap_values_sentiment = explainer_sentiment(texts_sample[:20])  # 20件でテスト
    
    # 授業評価スコア予測のSHAP分析
    print("📊 授業評価スコア予測のSHAP分析...")
    explainer_course = shap.Explainer(predict_course_only, tokenizer)
    shap_values_course = explainer_course(texts_sample[:20])  # 20件でテスト
    ...
    print("\n🎉 環境変数対応マルチタスクSHAP分析完了！")
    print(f"📁 結果は {output_dir} に保存されました")
    print(f"🔒 PyTorch 2.3.1のまま動作しました")
